#!/usr/bin/env python3
"""
Comprehensive Test Runner for Real LLM Integration
Tests OpenAI API with multiple spatial reasoning scenarios
"""
...
class RealLLMTester:
    """Comprehensive tester for real LLM integration"""
    ...
    def __init__(self, api_key: str = None):
        ...
        print(f"🧪 Real LLM Integration Tester")
        print(f"✅ Using OpenAI API: {self.api_key[:20]}...")
    ...
    def _test_scenario(self, scenario: Dict) -> List[TestResult]:
        ...
        for agent_id in range(1, 4):  # Test 3 agents per scenario
            ...
            decision = self._get_spatial_decision(agent_pos, target_pos, scenario["obstacles"])
            ...
            print(f"    Decision: {decision} | Success: {success} | Improvement: {improvement:.2f}")
...
Choose the action that moves you closest to the target while avoiding obstacles. Respond with ONLY the action name."""
        ...
        try:
            ...
            return self._fallback_decision(agent_pos, target_pos)
    ...
    def print_detailed_results(self, results: Dict):
        ...
        for i, result in enumerate(self.results[:10]):  # Show first 10 results
            print(f"  Test {i+1}: {result.decision} | Success: {result.success} | Improvement: {result.improvement:.2f}")
    
    def save_results(self, results: Dict, filename: str = "real_llm_test_results.json"):
...
def main():
    ...
    try:
        ...
        print(f"  - Real OpenAI API integration working")
