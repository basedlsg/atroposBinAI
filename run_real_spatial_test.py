#!/usr/bin/env python3
"""
Real Spatial Reasoning Test Runner
Uses actual Groq API to test LLM spatial reasoning capabilities
"""
...
class RealSpatialTestRunner:
    """Real test runner using actual Groq API"""
    ...
    async def test_single_spatial_reasoning(self, agent_id: str, scenario: str) -> dict:
    ...
    def _create_test_environment(self, scenario: str) -> dict:
        ...
        environments = {
            ...
            'spatial_memory': {
                ...
                'obstacles': [],
                'resources': [
                    {'position': (8, 6), 'type': 'resource'},
                    {'position': (12, 4), 'type': 'resource'}
                ],
...
Respond with JSON: {{"action": "move_direction", "reasoning": "brief explanation"}}"""
    ...
    async def _make_groq_request(self, prompt: str) -> dict:
    ...
    async def run_comprehensive_test(self, num_agents: int = 10) -> dict:
        ...
        for agent_id in range(num_agents):
            ...
            for scenario in scenarios:
                try:
                    ...
                    result = await self.test_single_spatial_reasoning(f"agent_{agent_id:03d}", scenario)
...
async def main():
    ...
    print("🧪 REAL SPATIAL REASONING TEST")
    print("=" * 50)
    ...
    runner = RealSpatialTestRunner()
    
    # Run comprehensive test
    print("\n🚀 Running comprehensive spatial reasoning test...")
    results = await runner.run_comprehensive_test(num_agents=5)  # Start with 5 agents
    
    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f"real_spatial_test_results_{timestamp}.json"
